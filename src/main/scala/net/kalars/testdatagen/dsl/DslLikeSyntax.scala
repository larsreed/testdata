package net.kalars.testdatagen.dsl

// Copyright (C) 2015 Lars Reed -- GNU GPL 2.0 -- see LICENSE.txt

import net.kalars.testdatagen.{BareGenerator, ExtendedGenerator, Generator, recordgen}
import net.kalars.testdatagen.aggreg.{FieldConcatenator, SomeNulls, TextWrapper, TwoFromFunction, TwoWithPredicate, UniqueWithFallback, WeightedGenerator}
import net.kalars.testdatagen.generators.misc.{CarMakes, CreditCards, Fibonaccis, Guids, MailAddresses, Markov, Names, Urls}
...
import net.kalars.testdatagen.generators.{Booleans, Chars, Dates, Doubles, Fixed, FromFile, FromList, FromStream, Ints, Longs, Strings, norway}
...
trait DslLikeSyntax {
  ...
  /** Alias Fnr(g). */
  def fnrFromDates(g: ExtendedGenerator[DateTime]): Fnr = norway.Fnr(g)
  /** Alias Orgnr(). */
  ...
  def toFile[T](fileName: String, noOfRecords: Int, append: Boolean=false, asStrings:Boolean= true,
    ...
    recordgen.ToFile(fileName, generator, append, charSet).write(noOfRecords, asStrings)
  /** Alias ToFixedWidth.*/
