...
class Dates extends ExtendedImpl[DateTime] {
  ...
  override def get(n: Int): List[DateTime] = {
    ...
    val min= if (!showTime)
                minOrg.withHourOfDay(stdHour).withMinuteOfHour(stdMin).withSecondOfMinute(stdSec).withMillisOfSecond(stdMilli)
             else if (!showDate)
                minOrg.withYear(stdYear).withMonthOfYear(stdMonth).withDayOfMonth(stdDay)
             else minOrg
    val max= if (!showTime)
                maxOrg.withHourOfDay(stdHour).withMinuteOfHour(stdMin).withSecondOfMinute(stdSec).withMillisOfSecond(stdMilli)
             else if (!showDate)
    ...
    def getSequentially: List[DateTime]= {
      @tailrec def next(last: DateTime, soFar:List[DateTime]): List[DateTime]=
        if (soFar.length>=n) soFar
          ...
          val nextVal= if (isReversed) d-stepPeriod else d + stepPeriod
